...
def obtener_url_base(request: Request) -> str:
    """Detecta automáticamente si la web está en Render (HTTPS) o en local."""
    host = request.headers.get("x-forwarded-host") or request.headers.get("host") or "127.0.0.1:8000"
    scheme = request.headers.get("x-forwarded-proto") or request.url.scheme or "https"
    # Si estamos en Render u otro hosting, siempre forzar https
...
async def guardar_evaluacion(request: Request, asignacion_id: int):
    ...
    # 29 indicadores * 3 pts máx = 87 pts = 100%
